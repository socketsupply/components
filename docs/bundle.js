...
class Tonic {
  ...
  _applyStyle () {
    if (this.style) {
      Object.entries(this.style()).forEach(pair => {
        if (pair[0] === '') return Object.assign(this.root.style, pair[1])
        Array.from(this.root.querySelectorAll(pair[0])).forEach(el => {
          Object.assign(el.style, pair[1])
        })
      })
    }
  }
    ...
    for (let { name, value } of this.root.attributes) {
      ...
      if (m && m[1]) {
        try {
          ...
          this.root.setAttribute(name, this.props[name])
        } catch (err) {
          this.props[name] = m[1]
