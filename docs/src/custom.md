...
class Example extends Tonic {
  ...
  // receive the Javascript event object.
  //
  mouseover (e) {
    // ...
  }
...
Each event handler method will receive the Javascript `event` object.
...
(which is unintuitive), it is a member of the class instance.
...
Sometimes you want to expose a method for other people to use. All of your
...
| Method | Description |
| :--- | :--- |
| `emit(String, Object)` | Emit a custom event on the root element of the component. A listener will receive a javascript event object that contains the [`detail`][4] property. |
